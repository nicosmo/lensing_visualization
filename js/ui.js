...
let uiLayer, toggleBtn;
let massSlider, spreadSlider, layersSlider, brightSlider, densitySlider;
let coreCheck, fgCheck, clusterCheck;
let massLabel, spreadLabel, coreLabel;
let btnVoid, btnPoint, btnNFW;
let btnGalaxies, btnBW, btnCol, btnDotted;
let layerList, bgUploadInput, uploadLabel, densityLabelText;
...
function updateLayerUI(config, material) {
    const manualLayers = LensingApp.manualLayers;
    layerList.innerHTML = '';
    ...
    if (manualLayers.length > 0) {
        ...
        if (LensingApp.currentMode === 'galaxies' || LensingApp.currentMode === 'dotted-grid') {
            densitySlider.disabled = false;
        }
    }
...
function setupResetButton(config, defaultConfig, material) {
    document.getElementById('reset-main').addEventListener('click', () => {
        ...
        if(fgCheck) { fgCheck.checked = true; config.showForeground = 1.0; }
        if(clusterCheck) { clusterCheck.checked = true; config.showCluster = 1.0; }
